...
params_data_tbl_summary <- params_data |>
  purrr::map(\(x) purrr::keep_at(x, init_cols)) |>
...
values for each strategy, and report back the `p10`, `p50` and `p90` values
for each of those distributions.

```{r create mixtures function}
create_mixtures <- function(tidy_intervals_tbl) {
  tidy_intervals_tbl |>
    dplyr::mutate(
...
```{r create mixtures across all runs}
#| warning: false
#| cache: true

final_table <- tidy_intervals_tbl |>
  create_mixtures() |>
...
final_table |>
  openxlsx2::write_xlsx("average_intervals_from_selected_tagged_runs.xlsx")
