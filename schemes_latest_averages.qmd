...
4. Intervals for all types of activity (inpatient, outpatient, aae) are
  included.
5. `p10`, `p50` and `p90` values are reported, taken from the distributions
  created from all interval values available for each TPMA, across all selected
  runs. Currently some intervals are only available from a small number of
  schemes, meaning that the basis for deriving "average" values is very narrow.
6. Out-of-range interval values provided by schemes are excluded. This includes
  four cases where schemes have somehow provided p10/p90 values greater than 1,
  and one case where a p10 value is given as 0.1 (ie 90% reduction in activity),
  which is deemed to be a user error.
...
tpma_lookup <- readr::read_csv(
  "reference/mitigator-lookup.csv",
  col_types = "c"
) |>
  dplyr::rename(
...
nee_intervals <- readr::read_rds("reference/nee_table.rds") |>
...
Table of values supplied by schemes that are contain either a zero value or a
value greater than 1:

```{r show extreme interval values}
#| filename: "show extreme interval values"
combined_intervals_tbl |>
  dplyr::filter(dplyr::if_any(c("p10", "p90"), \(x) x == 0 | x > 1))

```

### Visualising scheme-supplied intervals

This will help us notice supplied values that don't look right, and may cause
problems when creating the distributions (later) from which we will take the
"average" values.
...
# ONCE WE HAVE AN AGREED LIST OF SCHEME INTERVALS TO INCLUDE WE WILL MAKE THIS
# INTO A TABLE AND THEN FILTER combined_intervals_tbl AGAINST THIS USING
# anti_join(), INSTEAD OF THE CURRENT PROCESS BELOW
...
#   ) |>
#   dplyr::arrange(dplyr::desc(.data[["n_schemes"]]))
...
Because the provided intervals (`p10` and `p90` values) were supplied as projections of activity avoidance / efficiencies over different time spans (for
...
annualised_nee_intervals_tbl <- nee_intervals |>
  dplyr::mutate(
    annualised_p10 = annualise(.data[["p10"]], nee_period),
    annualised_p90 = annualise(.data[["p90"]], nee_period),
    .keep = "unused"
  )
...
mixture_dists_plotting_data |>
  dplyr::mutate(
    dplyr::across("strategy_name", \(x) stringr::str_wrap(x, 20))
  ) |>
  ggplot2::ggplot(ggplot2::aes(.data[["x_values"]], .data[["pdf_values"]])) +
  ggplot2::geom_line() +
  ggplot2::facet_wrap(~strategy_name, ncol = 4, scales = "free") +
...
pcols <- c("p10", "p50", "p90")
final_scheme_table <- scheme_mixtures_tbl |>
  dplyr::mutate(
  ...
  dplyr::rename_with(\(x) sub("^p", "schemes_p", x)) |>
  dplyr::mutate(dplyr::across("type", \(x) {
    forcats::fct(x, c("ip", "op", "aae"))
  })) |>
  dplyr::arrange(dplyr::pick(c("type", "change_factor", "strategy")))


pcols <- c("p10", "p90")
final_nee_table <- annualised_nee_intervals_tbl |>
  dplyr::rename_with(\(x) sub("^annualised_", "", x)) |>
  purrr::reduce(pcols, reinflate_5yr, .init = _) |>
  purrr::reduce(pcols, reinflate_7yr, .init = _) |>
  dplyr::select(!tidyselect::all_of(pcols)) |>
  dplyr::rename_with(\(x) sub("^p", "nee_p", x)) |>
  dplyr::mutate(dplyr::across("type", \(x) {
    forcats::fct(x, c("ip", "op", "aae"))
  })) |>
  dplyr::arrange(dplyr::pick(c("type", "change_factor", "strategy")))
...
```{r create joined table for schemes and nee figures}
#| filename: "create joined table for schemes and nee figures"

final_joined_table <- final_scheme_table |>
  dplyr::select(!c("strategy_name", "change_factor")) |>
  dplyr::left_join(final_nee_table, c("type", "strategy")) |>
  dplyr::select(!c("strategy", "change_factor")) |>
  dplyr::relocate("strategy_name", .after = "type")
...
final_joined_table |>
  dplyr::mutate(dplyr::across(tidyselect::matches("p[159]0"), \(x) round(x, 3)))
```
...
final_scheme_table |>
  readr::write_rds("truncated_composite_scheme_intervals.rds")

final_nee_table |>
  readr::write_rds("truncated_nee_intervals.rds")

final_scheme_table |>
  tidyr::pivot_longer(tidyselect::matches("p[159]0")) |>
  tidyr::separate("name", c("source", "name", "period")) |>
  tidyr::pivot_wider() |>
  readr::write_rds("truncated_composite_scheme_intervals.rds")

final_joined_table |>
  # fill in, by coalescing, missing NEE values first?
  tidyr::pivot_longer(
    tidyselect::matches("p[159]0"),
    names_to = c("source", "interval", "period"),
    names_sep = "_"
  ) |>
  readr::write_rds("nee_and_composite_scheme_intervals_long.rds")

final_nee_table |>
  tidyr::pivot_longer(tidyselect::matches("^p[159]0")) |>
  tidyr::separate("name", c("name", "period")) |>
  tidyr::pivot_wider() |>
  readr::write_rds("truncated_nee_intervals_longer.rds")

final_scheme_table |>
  openxlsx2::write_xlsx("truncated_composite_scheme_intervals.xlsx")
final_joined_table |>
  openxlsx2::write_xlsx("nee_and_composite_scheme_intervals.xlsx")
final_joined_table |>
  tidyr::pivot_longer(
    tidyselect::matches("p[159]0"),
    names_to = c("source", "interval", "period"),
  ...
  openxlsx2::write_xlsx("nee_and_composite_scheme_intervals_long.xlsx")
```
