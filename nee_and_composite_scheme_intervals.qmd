...
# 6 rows were identified through inspection to contain inapplicable values
remove_rows <- combined_intervals_tbl |>
  dplyr::filter(
    (grepl("^RH5", .data[["run_label"]]) &
      grepl("activity_avoidance_ari$", .data[["strategy"]])) |
      (grepl("^RAS", .data[["run_label"]]) &
        (grepl("adult_non-surgical$", .data[["strategy"]]) |
          grepl("pre-op_los_1-day", .data[["strategy"]])))
  ) |>
  dplyr::select(c("run_label", "strategy"))

# nrow(remove_rows) == 6 # check
...
# nrow(combined_intervals_tbl) - nrow(tidy_intervals_tbl) == 10 # check
...
final_joined_table <- inflation_periods |>
  ...
  # pivt_wider creates NA values in the table, where we are missing a NEE value
  tidyr::pivot_wider(
