...
nee_tbl <- final_nee_table |>
  dplyr::select(!c("strategy_name", "change_factor"))
# dplyr::select(!"change_factor")
final_joined_table <- final_scheme_table |>
  dplyr::select(!"change_factor") |>
  dplyr::left_join(nee_tbl, c("type", "strategy")) |>
  tidyr::pivot_longer(
    tidyselect::matches("p[159]0"),
    names_to = c("name", "interval", "period"),
    names_sep = "_"
  ) |>
  tidyr::pivot_wider() |>
  dplyr::mutate(dplyr::across("nee", \(x) {
    dplyr::coalesce(x, .data[["schemes"]])
  })) |>
  tidyr::pivot_longer(
    c("schemes", "nee"),
    names_to = "source",
  ...
  tidyr::pivot_wider(names_from = c("source", "period", "interval")) |>
  # NEE p50 values don't actually exist - introduced through pivoting
  dplyr::select(!tidyselect::matches("^nee.*p50$"))
...
final_joined_table |>
  dplyr::select(!tidyselect::ends_with("p50")) |>
  tidyr::pivot_longer(
